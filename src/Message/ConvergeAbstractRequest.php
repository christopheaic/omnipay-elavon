...
abstract class ConvergeAbstractRequest extends \Omnipay\Common\Message\AbstractRequest
{
    ...
    protected function getBaseData() : string
    {
        $data =
            ...
            '<ssl_user_id>' . $this->getUsername() . '</ssl_user_id>' .
            '<ssl_pin>' . $this->getPassword() . '</ssl_pin>' .
            '<ssl_test_mode>' . (($this->getTestMode() && !$this->getIntegrationTesting()) ? 'true' : 'false') . '</ssl_test_mode>' .
            '<ssl_show_form>' . (($this->getSslShowForm() && ($this->getSslShowForm() != 'false')) ? 'true' : 'false') . '</ssl_show_form>' .
            '<ssl_result_format>' . $this->getSslResultFormat() . '</ssl_result_format>' .
            '<ssl_invoice_number>' . $this->getSslInvoiceNumber() . '</ssl_invoice_number>';
